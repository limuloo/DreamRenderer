# Copyright 2024 The InstanceDiffusion Authors and HuggingFace Team. All rights reserved.
#
...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> images[0].save("./instancediffusion-1-5-generation.jpg")
        ```
"""
...
class StableDiffusionINSTDIFFPipeline(
    ...
    def check_inputs(
        ...
        if len(instdiff_phrases) != len(instdiff_boxes):
            raise ValueError(
                "length of `instdiff_phrases` and `instdiff_boxes` has to be same, but"
    ...
    def __call__(
        ...
        instdiff_scheduled_sampling_beta: float = 0.3,
        instdiff_phrases: List[str] = None,
        instdiff_boxes: List[List[float]] = None,
        ...
        Args:
            ...
            instdiff_phrases (`List[str]`):
                The phrases to guide what to include in each of the regions defined by the corresponding
                `instdiff_boxes`. There should only be one phrase per bounding box.
            instdiff_boxes (`List[List[float]]`):
        ...
        # Prepare InstanceDiffusion variables
        max_objs = 30
        if len(instdiff_boxes) > max_objs:
            warnings.warn(
        ...
        with self.progress_bar(total=len(timesteps) - num_beta_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps[num_beta_steps:]):
                ...
                if i_ == len(timesteps) - 1 or ((i_ + 1) > num_warmup_steps and (i_ + 1) % self.scheduler.order == 0):
                    progress_bar.update()
                    if callback is not None and i_ % callback_steps == 0:
                        step_idx = i_ // getattr(self.scheduler, "order", 1)
