...
class FluxRDIMGAttnProcessor2_0_CN_Union:
    ...
    def __call__(
        ...
        if FluxRDIMGAttnProcessor2_0_CN_Union.soft_bind_mask is None:
            atten_mask = torch.zeros(seq_len+HW, seq_len+HW, device=query.device)
            ...
            FluxRDIMGAttnProcessor2_0_CN_Union.soft_bind_mask = atten_mask

    


        if FluxRDIMGAttnProcessor2_0_CN_Union.hard_bind_mask is None:
            atten_mask = torch.zeros(seq_len+HW, seq_len+HW, device=query.device)
            for i in range(instance_num + 1):
                instance_text_idxs = instance_text_index_lst[i]
                bridge_image_token_index = bridge_image_token_index_list[i]
            ...
            for i in range(instance_num):
                ...
                atten_mask[(seq_len + instance_img_in_patch_idxs)[:, None], seq_len + instance_img_in_patch_idxs] = 0
            atten_mask = atten_mask.bool()
            FluxRDIMGAttnProcessor2_0_CN_Union.hard_bind_mask = atten_mask


        if FluxRDIMGAttnProcessor2_0_CN_Union.hard_bind_mask2 is None:
            atten_mask = torch.zeros(seq_len+HW, seq_len+HW, device=query.device)
            for i in range(instance_num + 1):
                instance_text_idxs = instance_text_index_lst[i]
                bridge_image_token_index = bridge_image_token_index_list[i]
            ...
            for i in range(instance_num):
                ...
                atten_mask[(seq_len + instance_img_in_patch_idxs)[:, None], instance_text_idxs] = 0
            atten_mask = atten_mask.bool()
            FluxRDIMGAttnProcessor2_0_CN_Union.hard_bind_mask2 = atten_mask
        
        if FluxRDIMGAttnProcessor2_0_CN_Union.counter % 15 in list(range(5, 10)):
            if now_steps < hard_control_steps:
                atten_mask = FluxRDIMGAttnProcessor2_0_CN_Union.hard_bind_mask
            else:
                atten_mask = FluxRDIMGAttnProcessor2_0_CN_Union.hard_bind_mask2
        else:
            atten_mask = FluxRDIMGAttnProcessor2_0_CN_Union.soft_bind_mask
        

        

        FluxRDIMGAttnProcessor2_0_CN_Union.counter += 1
        ...
        if FluxRDIMGAttnProcessor2_0_CN_Union.counter % (num_inference_steps * (15)) == 0:
            print('clear CN-Union attn mask')
            FluxRDIMGAttnProcessor2_0_CN_Union.counter = 0
            FluxRDIMGAttnProcessor2_0_CN_Union.soft_bind_mask = None
            FluxRDIMGAttnProcessor2_0_CN_Union.hard_bind_mask = None 
            FluxRDIMGAttnProcessor2_0_CN_Union.hard_bind_mask2 = None 
